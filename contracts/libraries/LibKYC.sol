...
library LibKYC {
    function _upgrade(address _address, IKYC.KYCLevel _level) internal {
        KYCStore storage kcs = KYCStorage.load();
        if (_level <= kcs.kyc[_address]) {
            revert IKYC.InvalidUpgrade();
        }
        kcs.kyc[_address] = _level;
        emit IKYC.KYCLevelUpgraded(_address, msg.sender, _level);
    }

    function _downgrade(address _address, IKYC.KYCLevel _level) internal {
        KYCStore storage kcs = KYCStorage.load();
        if (_level >= kcs.kyc[_address]) {
            revert IKYC.InvalidDowngrade();
        }
        kcs.kyc[_address] = _level;
        emit IKYC.KYCLevelDowngraded(_address, msg.sender, _level);
    }

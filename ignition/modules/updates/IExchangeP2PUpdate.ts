...
const IExchangeP2PUpdate = buildModule("IExchangeP2PUpdate", (m) => {
  ...
  const osC = [os, FacetCutAction.Replace, Object.values(osS)];
  ...
  const amC = [am, FacetCutAction.Replace, Object.values(amS)];

  const kyS = functionSelectors("KYC");
  const ky = m.contract("KYC");
  const kyC = [ky, FacetCutAction.Replace, Object.values(kyS)];
...
export default IExchangeP2PUpdate;

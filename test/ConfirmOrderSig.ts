...
describe("Confirm Order - OrderSig", function () {
  it("Buy[Confirm Order]", async function () {
    const {
      kofiMerchant,
      amaTrader,
      oneGrand,
    ...
    await expect(orderSigProxy.confirmOrder(payOrderMethod, payOrderSig))
      .to.emit(orderSigProxy, "OrderPaid")
      .withArgs(orderHash, OrderState.paid);
  });

  it("Buy[Reverts for Confirm Order]", async function () {
    const {
      kofiMerchant,
      amaTrader,
      yaaBrokie,
    ...
    await expect(
      orderSigProxy.confirmOrder(expiredPayOrderMethod, expiredPayOrderSig)
    ...
    await expect(
      orderSigProxy.confirmOrder(nonExistentPayOrderMethod, nonExistentPayOrderSig)
    ...
    await expect(
      orderSigProxy.confirmOrder(notTraderPayOrderMethod, notTraderPayOrderSig)
    ...
    await expect(
      orderSigProxy.confirmOrder(payOrderMethod, payOrderSig)
    ).to.be.revertedWithCustomError(orderSigProxy, "OrderAcceptedRequired");
  });

  it("Sell[Confirm Order - Pending State, Merchant Confirm]", async function () {
    const {
      kofiMerchant,
      amaTrader,
      oneGrand,
    ...
    await expect(
      orderSigProxy.confirmOrder(payOrderMethod, notMerchantpayOrderSig)
